...
最新源码预览：[2026.09.03 更新记录](docs/releases/2026-09-03.md) · [GitHub 发布页](https://github.com/AI-Max2000/CodexFloat/releases/tag/preview-2026-09-03)。本次提供源码，不包含已公证的正式安装包。
...
- 原生 `NSPanel + SwiftUI` 浮窗支持三种显示形态：`174×54` 完整额度条、`36×54` 热区内的竖向极简进度条，以及用竖向额度、百分比和倒计时组成的原生 macOS 菜单栏额度。前两种形态悬停后从固定左上角以液态胶囊动效展开为 `340px` 宽的完整浮窗，收起与展开内容共用同一动画进度；菜单栏形态悬停后在状态项下方打开完整内容。
...
- 「设置 → 额度显示 → 显示 5 小时额度」按实际返回的标准 Codex 窗口启用，不限制 Free、Plus、Pro 等套餐。检测到 5 小时与每周两个窗口时默认显示每周，开启后完整额度条与菜单栏并列显示两组读数，极简形态使用左侧 5 小时 / 右侧每周的双条；展开浮层分别显示百分比与重置时间。没有 5 小时窗口则不显示该开关；只有 5 小时窗口则保持单额度显示，开关锁定并解释原因。旧版开关选择会迁移保留，不改变 GPT 专项额度与通知规则，不生成不存在的周或月额度。
