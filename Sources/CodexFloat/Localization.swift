...
enum LocalizedTextKey: String, CaseIterable {
  ...
  case globalReset, bankedReset, conditionalReset, limitChange, plannedActivity, incidentOrFix, other
...
struct AppStrings: Sendable {
  ...
  private static let table: [LocalizedTextKey: (hans: String, hant: String, en: String)] = [
    ...
    .languageDescription: ("切换后立即生效，包括浮窗、菜单、设置和之后的通知；中文时间使用北京时间，英文时间使用美国太平洋时间。", "切換後立即生效，包括懸浮視窗、選單、設定和之後的通知；中文時間使用北京時間，英文時間使用美國太平洋時間。", "Changes apply immediately to the floating window, menus, settings, and future notifications. Chinese uses Beijing Time; English uses U.S. Pacific Time."),
    ...
    .frontmostDisabledHelp: ("浮窗会继续显示在其他应用上方。", "懸浮視窗會繼續顯示在其他應用程式上方。", "The floating window stays above other apps."),
    .menuBarAlwaysVisibleHelp: ("菜单栏额度始终显示，不受前台应用限制；这个开关只作用于完整额度和极简进度条。", "選單列額度會持續顯示，不受前景應用程式限制；此開關只套用於完整額度和極簡進度列。", "Menu bar quota always stays visible, regardless of the frontmost app. This setting only affects Full quota and Minimal meter modes."),
    ...
    .shortcutHelp: ("点击快捷键框后直接按新组合。至少包含 Control、Option 或 Command；按 Esc 取消。", "點擊快捷鍵框後直接按新組合。至少包含 Control、Option 或 Command；按 Esc 取消。", "Click the shortcut field and press a new combination. Include Control, Option, or Command; press Esc to cancel."),
    ...
    .taskHelp: ("每 2 秒只读检查任务状态：绿色为完成或空闲，黄色为进行中，红色为失败。不读取聊天正文。", "每 2 秒以唯讀方式檢查任務狀態：綠色為完成或閒置，黃色為進行中，紅色為失敗。不讀取對話內容。", "Checks task state read-only every 2 seconds: green means complete or idle, yellow means working, and red means failed. Chat content is never read."),
    ...
    .expiringResetCreditAlert: ("额外重置次数将在 48 小时内过期", "額外重置次數將在 48 小時內過期", "Extra reset expires within 48 hours"),
    ...
    .autoRefreshHelp: ("Codex 额度变化时立即刷新；没有事件时按所选间隔校准。失败后每 10 秒自动重试。", "Codex 額度變化時立即更新；沒有事件時按所選間隔校準。失敗後每 10 秒自動重試。", "Refreshes immediately when Codex reports a quota change and checks at the selected interval otherwise. Retries every 10 seconds after a failure."),
    .showSupplementaryQuotas: ("显示 GPT-5.3 和 GPT 备用额度", "顯示 GPT-5.3 和 GPT 備用額度", "Show GPT-5.3 and GPT reserve quotas"),
    .supplementaryQuotaHelp: ("默认只显示标准 Codex 额度。开启后额外显示 GPT-5.3-Codex-Spark 的主、次窗口以及 GPT 备用额度。", "預設只顯示標準 Codex 額度。開啟後另外顯示 GPT-5.3-Codex-Spark 的主、次視窗與 GPT 備用額度。", "By default, only standard Codex quotas are shown. Enable this to also show GPT-5.3-Codex-Spark primary and secondary windows and GPT reserve quota."),
    .tiboSection: ("Tibo 重置消息", "Tibo 重置消息", "Tibo reset updates"),
    .tiboPolling: ("每 5 分钟检查，失败后自动降低频率", "每 5 分鐘檢查，失敗後自動降低頻率", "Check every 5 minutes and back off after failures"),
    .tiboHelp: ("只显示与额度重置直接相关的公告和预计时间。", "只顯示與額度重置直接相關的公告與預計時間。", "Shows only announcements directly related to quota resets and their expected time."),
    .resetProbabilityToggle: ("显示未来 48 小时重置概率", "顯示未來 48 小時重置機率", "Show 48-hour reset probability"),
    .resetProbabilityHelp: ("每 5 分钟自动获取最新预测，并结合近期重置节奏、Tibo 明确信号、数据新鲜度和用户里程碑。用户增长只作证据：“每增加 100 万就重置”的公开承诺已在 1000 万时结束。", "每 5 分鐘自動取得最新預測，並結合近期重置節奏、Tibo 明確訊號、資料新鮮度與用戶里程碑。用戶成長只作證據：「每增加 100 萬就重置」的公開承諾已在 1000 萬時結束。", "Fetches the latest forecast every 5 minutes and combines recent reset cadence, explicit Tibo signals, source freshness, and user milestones. Growth is evidence only: the public every-1M promise ended at 10M users."),
    .privacySection: ("隐私与诊断", "隱私與診斷", "Privacy and diagnostics"),
    .privacyHelp: ("只读取本机 Codex 的额度和任务状态；不读取登录凭据、聊天正文、浏览器登录信息或 Codex 运行记录。", "只讀取本機 Codex 的額度和任務狀態；不讀取登入憑證、對話內容、瀏覽器登入資訊或 Codex 執行記錄。", "Reads only local Codex quota and task status. It never reads credentials, chat content, browser sessions, or Codex rollout logs."),
    .exportDiagnostics: ("导出脱敏诊断…", "匯出去識別化診斷…", "Export redacted diagnostics…"),
    .refresh: ("刷新", "更新", "Refresh"),
    .settings: ("设置", "設定", "Settings"),
    .hide: ("隐藏", "隱藏", "Hide"),
    .hideHelp: ("隐藏浮窗；可用全局快捷键或重新打开应用恢复", "隱藏懸浮視窗；可用全域快捷鍵或重新開啟應用程式恢復", "Hide the window; use the global shortcut or reopen the app to restore it"),
    ...
    .refreshFailedShowingPrevious: ("刷新失败 · 显示 %@ 数据", "更新失敗 · 顯示 %@ 資料", "Refresh failed · showing %@ data"),
    ...
    .noTiboResetAnnouncement: ("暂无新的 Tibo 重置公告", "暫無新的 Tibo 重置公告", "No new reset announcement from Tibo"),
    .expectedReset: ("预计重置：%@ · %@", "預計重置：%@ · %@", "Expected reset: %@ · %@"),
    .expectedResetTimeLine: ("预计重置：%@", "預計重置：%@", "Expected reset: %@"),
    .effectiveWhenAnnounced: ("公告时已生效（约 %@）", "公告時已生效（約 %@）", "Effective when announced (about %@)"),
    .pendingConfirmation: ("待确认", "待確認", "To be confirmed"),
    .resetProbability48Hours: ("未来 48 小时重置概率 %d%%", "未來 48 小時重置機率 %d%%", "%d%% reset probability in the next 48 hours"),
    .resetProbabilityCalculating: ("正在计算重置概率…", "正在計算重置機率…", "Calculating reset probability…"),
    .resetProbabilityExpired: ("预测数据已过期，等待刷新", "預測資料已過期，等待更新", "Forecast expired; waiting for refresh"),
    ...
    .forecastNextMillion: ("按近期增长速度，%@M 约在 %@（仅作增长参考）", "按近期成長速度，%@M 約在 %@（僅作成長參考）", "At recent growth speed, %@M is estimated around %@ (growth context only)"),
    .forecastNextMajorMilestone: ("下一个重要里程碑 %@M 约在 %@", "下一個重要里程碑 %@M 約在 %@", "Next major %@M milestone is estimated around %@"),
    .forecastMillionPromiseActive: ("历史“每增加 100 万重置”承诺仍在 1000 万范围内。", "歷史「每增加 100 萬重置」承諾仍在 1000 萬範圍內。", "The historical every-1M reset promise is still within its up-to-10M scope."),
    .forecastMillionPromiseEnded: ("每增加 100 万就重置的承诺已在 1000 万结束；之后的增长只作参考，不单独加概率。", "每增加 100 萬就重置的承諾已在 1000 萬結束；之後的成長只作參考，不單獨增加機率。", "The every-1M promise ended at 10M; later growth is context and does not add probability by itself."),
    .forecastNoMilestone: ("暂无可验证的用户里程碑数据。", "暫無可驗證的用戶里程碑資料。", "No verifiable user-milestone data is available."),
    .forecastUpdatedAt: ("预测数据更新于 %@", "預測資料更新於 %@", "Forecast data updated %@"),
    .forecastDisclaimer: ("这是非官方实验性预测，不是 OpenAI 承诺；工作计划仍应以当前可用额度为准。", "這是非官方實驗性預測，不是 OpenAI 承諾；工作計畫仍應以目前可用額度為準。", "This is an unofficial experimental forecast, not an OpenAI promise. Plan work around the quota currently available."),
    ...
    .activitySummaryGlobal: ("Tibo 宣布相关用户的额度已统一重置，仍需核对本机账号变化。", "Tibo 宣布相關用戶的額度已統一重置，仍需核對本機帳號變化。", "Tibo announced a global quota reset for the relevant users; local account changes still need verification."),
    .activitySummaryBanked: ("Tibo 宣布发放可自行使用的额外重置次数，是否到账以本机账号变化为准。", "Tibo 宣布發放可自行使用的額外重置次數，是否到帳以本機帳號變化為準。", "Tibo announced an extra reset you can use manually; local account changes determine whether it arrived."),
    .activitySummaryConditional: ("这是需要满足条件的重置奖励，不代表当前账号可直接重置。", "這是需要滿足條件的重置獎勵，不代表目前帳號可直接重置。", "This reset reward requires conditions and does not mean your account can reset immediately."),
    .activitySummaryLimit: ("Tibo 提到了额度窗口、套餐或消耗规则变化，以账号实际额度为准。", "Tibo 提到了額度視窗、方案或用量規則變化，以帳號實際額度為準。", "Tibo mentioned changes to quota windows, plans, or usage rules; your account quota is the source of truth."),
    .activitySummaryPlanned: ("Tibo 预告了重置或活动，目前尚未确认在你的账号生效。", "Tibo 預告了重置或活動，目前尚未確認在你的帳號生效。", "Tibo previewed a reset or event that has not yet been confirmed on your account."),
    .activitySummaryIncident: ("Tibo 提到额度异常、修复或服务状态；这不是可用重置次数的确认。", "Tibo 提到額度異常、修復或服務狀態；這不是可用重置次數的確認。", "Tibo mentioned a quota issue, fix, or service status; this is not confirmation of an available reset."),
    .activitySummaryOther: ("这条帖子没有明确、可操作的额度重置信息。", "這則貼文沒有明確、可操作的額度重置資訊。", "This post has no clear, actionable quota reset information."),
    .activityHistoryTitle: ("Tibo 重置记录", "Tibo 重置記錄", "Tibo reset history"),
    .noResetAnnouncements: ("暂无重置公告", "暫無重置公告", "No reset announcements"),
    .unrelatedPostsHidden: ("与额度重置无关的 Tibo 帖子不会显示在这里", "與額度重置無關的 Tibo 貼文不會顯示在這裡", "Tibo posts unrelated to quota resets are hidden here"),
    ...
    .sourceStatus: ("来源：%@ · 更新：%@ · 只显示重置公告", "來源：%@ · 更新：%@ · 只顯示重置公告", "Source: %@ · Updated: %@ · Reset announcements only"),
    ...
    .countOnlyNoDetails: ("服务只返回了数量，没有可展示的详情。", "服務只回傳了數量，沒有可顯示的詳情。", "The service returned a count but no displayable details."),
    .resetCreditDefaultTitle: ("额外重置次数", "額外重置次數", "Extra reset"),
    .status: ("状态：%@", "狀態：%@", "Status: %@"),
    .creditedAt: ("到账：%@", "到帳：%@", "Credited: %@"),
    .expiresAt: ("过期：%@", "過期：%@", "Expires: %@"),
    .readOnlyResetHelp: ("只读模式：Codex Float 不会自动使用额外重置次数。", "唯讀模式：Codex Float 不會自動使用額外重置次數。", "Read-only mode: Codex Float never uses an extra reset automatically."),
    ...
    .lastSuccessfulUpdate: ("%@ · %@ · 最后成功更新 %@", "%@ · %@ · 最後成功更新 %@", "%@ · %@ · Last successful update %@"),
    ...
    .menuBarQuotaHelp: ("Codex 剩余 %d%%，%@ 刷新。悬停查看完整内容。", "Codex 剩餘 %d%%，%@ 更新。懸停查看完整內容。", "Codex has %d%% remaining and refreshes %@. Hover for full details."),
    ...
    .notifyResetAddedBody: ("已在你的账号观察到额外重置次数从 %d 增加到 %d。", "已在你的帳號觀察到額外重置次數從 %d 增加到 %d。", "Extra resets on your account increased from %d to %d."),
    .notifyResetExpiryTitle: ("额外重置次数即将过期", "額外重置次數即將過期", "Extra reset expires soon"),
    .notifyResetExpiryBody: ("%@ 将在 48 小时内过期。助手不会自动使用它。", "%@ 將在 48 小時內過期。助手不會自動使用它。", "%@ expires within 48 hours. The assistant will not use it automatically."),
    .notifyTiboFiveHourTitle: ("Tibo 活动将在 5 小时后生效", "Tibo 活動將在 5 小時後生效", "Tibo event starts in 5 hours"),
    .notifyTiboBody: ("对象：%@ · 时间：%@。%@", "對象：%@ · 時間：%@。%@", "Audience: %@ · Time: %@. %@"),
    .notifyTiboFiveHourBody: ("对象：%@ · %@ · 预计 %@", "對象：%@ · %@ · 預計 %@", "Audience: %@ · %@ · Expected %@"),
    .notifyTaskCompletedTitle: ("Codex 任务已完成", "Codex 任務已完成", "Codex task completed"),
    .notifyQuotaWithinFiveHours: ("Codex 额度将在 5 小时内刷新", "Codex 額度將在 5 小時內更新", "Codex quota refreshes within 5 hours"),
    .notifyQuotaInFiveHours: ("Codex 额度将在 5 小时后刷新", "Codex 額度將在 5 小時後更新", "Codex quota refreshes in 5 hours"),
    .notifyQuotaResetBody: ("%@ 将在 %@ 刷新，当前剩余 %d%%。", "%@ 將在 %@ 更新，目前剩餘 %d%%。", "%@ refreshes at %@ and currently has %d%% remaining."),
    .timingAlreadyEffective: ("原帖称已经生效，公告时间为 %@", "原文表示已經生效，公告時間為 %@", "The original post says it was already effective; announced at %@"),
    ...
    .feedbackTiboFuture: ("适用对象：%@。预计 %@ 可以重置额度。", "適用對象：%@。預計 %@ 可以重置額度。", "For %@. Quota is expected to reset at %@."),
    .feedbackTiboEffective: ("适用对象：%@。Tibo 称重置已于 %@ 生效，正在等待本机额度验证。", "適用對象：%@。Tibo 表示重置已於 %@ 生效，正在等待本機額度驗證。", "For %@. Tibo says the reset took effect at %@; waiting for local quota verification."),
    .feedbackTiboUnknown: ("适用对象：%@。Tibo 已预告重置，但暂未给出明确时间。", "適用對象：%@。Tibo 已預告重置，但暫未提供明確時間。", "For %@. Tibo previewed a reset but has not provided an exact time."),
    .feedbackTiboCallout: ("请尽情吩咐 Codex 吧～", "請盡情吩咐 Codex 吧～", "Feel free to put Codex to work!"),
    .feedbackTiboUnknownCallout: ("时间确认后会再次提醒你。", "時間確認後會再次提醒你。", "You will be alerted again when the time is confirmed."),
    ...
    .feedbackQuotaMessage: ("当前剩余 %d%%，预计 %@ 刷新。", "目前剩餘 %d%%，預計 %@ 更新。", "%d%% remains and is expected to refresh at %@."),
    .feedbackQuotaLowCallout: ("建议先安排重要任务。", "建議先安排重要任務。", "Consider prioritizing important tasks."),
    .feedbackQuotaCriticalCallout: ("请优先完成正在进行的任务。", "請優先完成正在進行的任務。", "Prioritize tasks already in progress."),
    ...
    .feedbackMenuQuotaLow: ("Codex 偏低 · %d%%", "Codex 偏低 · %d%%", "Codex low · %d%%"),
    .feedbackMenuQuotaCritical: ("Codex 即将用尽 · %d%%", "Codex 即將用盡 · %d%%", "Codex critical · %d%%"),
    ...
    .previewFeedbackHelp: ("切换显示形态后，可在这里直接检查对应的颜色、动效和提示文案。", "切換顯示形態後，可在這裡直接檢查對應的顏色、動效與提示文字。", "Switch display modes, then use these buttons to check each color, animation, and message."),
    ...
    .errorForecastUnavailable: ("重置概率来源暂不可用：%@", "重置機率來源暫時無法使用：%@", "Reset forecast source unavailable: %@"),
