...
struct FloatingPanelLayoutTests {
  ...
  @Test @MainActor func movingExpandedPanelUpdatesTheSharedAnchorBeforeSmoothCollapse() async throws {
    ...
    #expect(Set(initialExpansionFrames.map { Int($0.width.rounded()) }).count >= 8)
    ...
    #expect(controller.isFrameTransitionInFlight)
    #expect(interruptedExpansionFrame.width > draggedRestingFrame.width)
    #expect(interruptedExpansionFrame.width < 340)
