...
- 开启“跟随 Codex 窗口移动”时，通过 macOS 窗口列表读取 Codex 的进程 ID、窗口 ID、可见性和边界，并观察鼠标按下/拖动/松开事件；检测到窗口移动后暂时隐藏工具，继续读取目标位置以便停稳后恢复。不读取按键、窗口标题或画面，不请求辅助功能或屏幕录制权限。仅在本机偏好中保存两种浮窗形态的相对位置，不保存窗口或进程 ID。手动隐藏、前台显示规则隐藏、功能关闭或切到菜单栏模式时停止跟踪；移动期间的临时隐藏不改变用户的显示偏好。
